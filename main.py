...
class NovaPoshtaBot:
    ...
    def create_counterparty(self, name, phone, is_sender=True):
        counterparty_ref, contact_ref = self.find_counterparty(phone, is_sender)
    ...
    def create_ttn(self, parsed_data):
        try:
            ...
            if not sender_ref or not contact_sender_ref:
                return "Ошибка: Не удалось создать или найти отправителя"
            ...
            if not recipient_ref or not contact_recipient_ref:
                return "Ошибка: Не удалось создать или найти получателя"
...
def main():
    TELEGRAM_TOKEN = os.getenv("7407440639:AAHd2CUJJVMjdbeeYnzq7kBVNGnVhZFWJOQ")
    NOVA_POSHTA_API_KEY = os.getenv("cb589626abe2488ac0bd2c750419a496")

    print(f"TELEGRAM_TOKEN: {TELEGRAM_TOKEN}")
    print(f"NOVA_POSHTA_API_KEY: {NOVA_POSHTA_API_KEY}")
